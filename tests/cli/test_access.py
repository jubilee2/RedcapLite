...
from redcaplite.cli import setup as setup_cli_module
from redcaplite.cli.setup import SetupCommand
...
def test_main_setup_command_creates_profile(tmp_path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr("redcaplite.cli.setup.prompt_text", lambda _: "https://redcap.example.edu/api/")
...
def test_setup_command_rejects_invalid_token(tmp_path, monkeypatch, capsys) -> None:
    profile_store = ProfileStore(tmp_path)
    token_store = TokenStore(tmp_path)
    ...
    monkeypatch.setattr("redcaplite.cli.setup.prompt_confirm", lambda _: False)
    monkeypatch.setattr("redcaplite.cli.setup.prompt_secret", lambda _: "bad-token")
...
def test_setup_command_keeps_existing_token_when_user_declines_replacement(
    ...
    monkeypatch.setattr("redcaplite.cli.setup.prompt_confirm", fake_confirm)
...
def test_main_setup_command_keeps_url_and_replaces_token(tmp_path, monkeypatch, capsys) -> None:
    profile_store = ProfileStore(tmp_path)
    token_store = TokenStore(tmp_path)
    profile_store.upsert(Profile(name="demo", url="https://redcap.example.edu/api/"))
...
def test_main_setup_command_updates_url_and_token(tmp_path, monkeypatch, capsys) -> None:
    profile_store = ProfileStore(tmp_path)
    token_store = TokenStore(tmp_path)
    profile_store.upsert(Profile(name="demo", url="https://redcap.example.edu/api/"))
    token_store.save_token("demo", "old-token")
    ...
    monkeypatch.setattr("redcaplite.cli.setup.prompt_confirm", fake_confirm)
    monkeypatch.setattr("redcaplite.cli.setup.prompt_text", lambda _: "https://updated.example.edu/api/")
    monkeypatch.setattr("redcaplite.cli.setup.prompt_secret", lambda _: "new-token")
    monkeypatch.setattr("redcaplite.cli.setup.SetupCommand", lambda: command)
...
def test_run_setup_rejects_invalid_url(tmp_path, monkeypatch, capsys) -> None:
    profile_store = ProfileStore(tmp_path)
    token_store = TokenStore(tmp_path)
    monkeypatch.setattr("redcaplite.cli.setup.prompt_text", lambda _: "redcap.example.edu/api/")

    assert (
        setup_cli_module.run_setup(
            "demo",
            profile_store=profile_store,
            token_store=token_store,
            client_factory=FakeClient,
...
def test_run_setup_keeps_existing_token_when_replacement_declined(tmp_path, monkeypatch, capsys) -> None:
    profile_store = ProfileStore(tmp_path)
    token_store = TokenStore(tmp_path)
    profile_store.upsert(Profile(name="demo", url="https://redcap.example.edu/api/"))
...
def test_run_setup_updates_existing_profile_token(tmp_path, monkeypatch, capsys) -> None:
    profile_store = ProfileStore(tmp_path)
    token_store = TokenStore(tmp_path)
    profile_store.upsert(Profile(name="demo", url="https://redcap.example.edu/api/"))
    token_store.save_token("demo", "existing-token")
    prompt_answers = iter([False, True])
    monkeypatch.setattr("redcaplite.cli.setup.prompt_confirm", lambda _: next(prompt_answers))
    monkeypatch.setattr("redcaplite.cli.setup.prompt_secret", lambda _: "new-token")
