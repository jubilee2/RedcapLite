...
def test_response_error_handler_400():
    ...
    mock_func = Mock(return_value=Mock(status_code=400, json=lambda: {'error': 'Test error message'}))
    decorated_func = response_error_handler(mock_func)
...
def test_response_error_handler_401():
    ...
    with pytest.raises(APIException) as exc_info:
        decorated_func(None, {})
    assert str(exc_info.value) == "Unauthorized: API token was missing or incorrect."
...
def test_response_error_handler_403():
    ...
    with pytest.raises(APIException) as exc_info:
        decorated_func(None, {})
    assert str(exc_info.value) == "Forbidden: You do not have permissions to use the API."
...
def test_response_error_handler_404():
    ...
    with pytest.raises(APIException) as exc_info:
        decorated_func(None, {})
    assert str(exc_info.value) == "Not Found: The URI requested is invalid or the resource does not exist."
...
def test_response_error_handler_406():
    ...
    with pytest.raises(APIException) as exc_info:
        decorated_func(None, {})
    assert str(exc_info.value) == "Not Acceptable: The data being imported was formatted incorrectly."
...
def test_response_error_handler_500():
    ...
    with pytest.raises(APIException) as exc_info:
        decorated_func(None, {})
    assert str(exc_info.value) == "Internal Server Error: The server encountered an error processing your request."
...
def test_response_error_handler_501():
    ...
    with pytest.raises(APIException) as exc_info:
        decorated_func(None, {})
    assert str(exc_info.value) == "Not Implemented: The requested method is not implemented."
...
def test_file_download_handler():
    ...
    mock_func = Mock(return_value=Mock(headers={"content-type": 'application/octet-stream; name="test.txt"'}))
...
def test_file_download_handler_no_name_in_content_type():
    ...
    mock_func = Mock(return_value=Mock(headers={"content-type": 'application/octet-stream'}))
...
def test_file_upload_handler():
    ...
    with patch('builtins.open', new=fake_open):
        ...
        assert response == mock_func.return_value
        mock_func.assert_called_once_with(None, {}, files={'file': fake_open.return_value})
