...
class RedcapClient(Client):
    ...
    def import_arms(self, data: List[Dict[str, Any]]):
    ...
    def import_dags(self, data: List[Dict[str, Any]]):
    ...
    def import_user_dag_mappings(self, data: List[Dict[str, Any]]):
        return self.post(api.import_user_dag_mappings({'data': data}))

    # events
    def get_events(self, arms: Optional[List[int]] = []):
        return self.post(api.get_events({'arms': arms}))

    def import_events(self, data: List[Dict[str, Any]]):
    ...
    # file
    def get_file(self, file_dictionary: str = '', **kwargs):
        return self.file_download_api(api.get_file(kwargs), file_dictionary=file_dictionary)

    def import_file(self, file_path: str, **kwargs):
