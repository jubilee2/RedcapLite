...
- `sync profile2 [--yes]` exports metadata from both profiles, uses `field_name` and `form_name` to align matching rows, prints source-only/target-only rows with all metadata columns, shows aligned row differences side-by-side across the remaining columns, and then optionally imports the source profile metadata into the target profile
